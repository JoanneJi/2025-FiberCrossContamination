...
after looping through all traces in one observation with SNR set as 100 and 800, and sci-sci frac ranging from 1e-5 to 1e-1.
Parallel processing for SNR and sci-sci frac.
Only sci-sci cross-contamination is added here.
...
tasks = []
for scisci_frac in scisci_frac_values:
...
def save_file(params: namedtuple, params_inst: namedtuple, params_flags: namedtuple) -> None:
    ...
    for ispec in range(params.nspec):
        for iband in params_inst.band:
            ...
            # save the results
            # example of filename: ./output/synfile_scisci/snr100/1e-5_0_red.npz
            filename = f"{params.scisci_frac}_{ispec}_{iband}.npz"
            filepath = os.path.join(pwd, f'./output/synfile_scisci/snr{params.SNR_sci}/{filename}')
...
if __name__ == '__main__':
    ...
    # Run multiprocessing
    num_processes = len(scisci_frac_values) * len(SNR_values)  # Number of processes
