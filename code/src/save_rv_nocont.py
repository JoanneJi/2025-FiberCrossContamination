...
after looping through all traces in one observation with SNR set from 100 to 800 and R set as 90k, 120k, 150k.
Parallel processing for R and SNR.
No cross-contamination is added here.
...
for R in R_values:
    for SNR in SNR_values:
        params = Arg(
            ...
            trace_height=17,
            # observation info
            SNR_cal=SNR,  # change here if you want to adjust the brightness of the cal line
            SNR_sci=SNR,
...
def save_file(params: namedtuple, params_inst: namedtuple, params_flags: namedtuple) -> None:
    ...
    for ispec in range(params.nspec):
        for iband in params_inst.band:
            ...
            # save the results
            # example of filename: ./output/synfile_nocont/snr100/120000_0_red.npz
            filename = f"{params.R}_{ispec}_{iband}.npz"
            filepath = os.path.join(pwd, f'./output/synfile_nocont/snr{params.SNR_sci}/{filename}')
...
if __name__ == '__main__':
    ...
    # Run multiprocessing
    num_processes = len(R_values) * len(SNR_values)  # Number of processes
